...
Thực nghiệm tiến hành chạy các thuật toán trên cùng kịch bản **Đăng ký tài khoản (User Sign Up)**. Để giải thích sự kết hợp tối ưu giữa LLM, GA và HC, tiến trình dữ liệu được vận hành theo luồng (Pipeline) dưới đây:

### 3.6.0. Sơ đồ luồng dữ liệu (Data Pipeline)
Dữ liệu đặc tả BRD ban đầu được gửi qua LLM để sinh dữ liệu thô (F0), sau đó đi qua giải thuật di truyền GA để thực hiện lai ghép, đột biến phạm vi rộng (toàn cục), cuối cùng đi qua thuật toán leo đồi HC để tinh chỉnh biên (cục bộ):

$$\text{Đặc tả BRD} \longrightarrow \fbox{LLM} \xrightarrow{\text{Sinh data thô (F0)}} \fbox{GA} \xrightarrow{\text{Lai ghép/Đột biến}} \fbox{HC} \longrightarrow \text{Bộ Test Case Tối Ưu}$$

Hoặc biểu diễn bằng sơ đồ khối Mermaid:
```mermaid
graph LR
    BRD[Đặc tả BRD] -->|Phân tích & Sinh thô| LLM[Mô hình LLM]
    LLM -->|Tập dữ liệu F0| GA[Giải thuật Di truyền - Tìm kiếm toàn cục]
    GA -->|Tập cá thể Elite| HC[Leo đồi biên - Tinh chỉnh cục bộ]
    HC -->|Tập Test Case tối ưu| Output[Bộ Test Case Hoàn Thiện]
```

### 3.6.1. Kết quả LLM
Kết quả đầu tiên của hệ thống là tập test data F0 được sinh sau khi LLM phân tích đặc tả yêu cầu. So với phương pháp sinh ngẫu nhiên, dữ liệu do LLM tạo ra có ý nghĩa nghiệp vụ rõ ràng hơn vì bám sát các trường dữ liệu và ràng buộc trong đặc tả.

Với kịch bản thêm sản phẩm, LLM có thể sinh các test case như sản phẩm hợp lệ, thiếu tên sản phẩm, giá bằng 0, giá âm, số lượng âm, mã sản phẩm sai định dạng hoặc danh mục không hợp lệ. Các test case này phản ánh được các tình huống cơ bản cần kiểm thử.

Tuy nhiên, F0 vẫn có thể chưa tối ưu hoàn toàn. Một số test case có thể bị trùng lặp, chưa đủ đa dạng hoặc chưa bao phủ hết các giá trị biên. Vì vậy, tập F0 tiếp tục được đưa vào bước tối ưu bằng GA.

### 3.6.2. Kết quả GA
Sau khi có tập F0, hệ thống áp dụng GA để tối ưu toàn cục. GA đánh giá từng test case bằng hàm fitness, sau đó thực hiện chọn lọc, lai ghép và đột biến để tạo ra các thế hệ dữ liệu mới.

Kết quả sau GA cho thấy tập test data có xu hướng đa dạng hơn so với tập dữ liệu ban đầu. Các test case trùng lặp hoặc có điểm fitness thấp dần bị loại bỏ. Trong khi đó, các test case có điểm fitness cao được giữ lại và tiếp tục tham gia tạo thế hệ mới.

Ngoài ra, GA có thể tạo ra các tổ hợp dữ liệu mà LLM chưa sinh ở tập F0. Ví dụ, từ các test case riêng lẻ như sai giá hoặc sai mã sản phẩm, GA có thể tạo ra test case kết hợp nhiều điều kiện kiểm thử trong cùng một bộ dữ liệu.

### 3.6.3. Kết quả GA kết hợp HC
Kết quả GA kết hợp HC là bộ test data cuối cùng sau khi đã trải qua cả hai giai đoạn tối ưu. GA đảm nhiệm vai trò tìm kiếm rộng và cải thiện chất lượng tổng thể của tập dữ liệu. HC tiếp tục tinh chỉnh chi tiết các test case tốt nhất, đặc biệt là các giá trị tại biên hoặc gần biên.

Sau khi áp dụng HC, bộ test data cuối cùng có khả năng kiểm tra tốt hơn các trường hợp dễ phát sinh lỗi. Ví dụ, với trường `price`, HC có thể tạo ra các giá trị như -1, 0 và 1 để kiểm tra điều kiện giá phải lớn hơn 0. Với trường `productName`, HC có thể tinh chỉnh độ dài chuỗi về các giá trị ngay dưới biên, đúng biên và vượt biên.

So với kết quả chỉ dùng LLM, bộ test data sau GA + HC có tính đa dạng cao hơn và kiểm tra sâu hơn các trường hợp biên. So với kết quả chỉ dùng GA, bộ test data sau GA + HC có khả năng tinh chỉnh tốt hơn tại các vùng dữ liệu nhạy cảm.
...
Để đánh giá hiệu quả của hệ thống, các phương pháp sinh dữ liệu được so sánh dựa trên các tiêu chí như số lượng test data hợp lệ, số lượng test data không hợp lệ, khả năng bao phủ giá trị biên, mức độ đa dạng, tỷ lệ trùng lặp và thời gian xử lý.

Phương pháp sinh dữ liệu ngẫu nhiên có ưu điểm là đơn giản và dễ triển khai. Tuy nhiên, dữ liệu sinh ngẫu nhiên thường thiếu ý nghĩa nghiệp vụ, không bám sát đặc tả và dễ bỏ sót các tình huống quan trọng.

Phương pháp sử dụng LLM giúp cải thiện chất lượng dữ liệu ban đầu. Dữ liệu do LLM sinh ra bám sát đặc tả hơn, có thể bao gồm dữ liệu hợp lệ, không hợp lệ, thiếu trường bắt buộc, sai định dạng và một số dữ liệu biên. Tuy nhiên, LLM vẫn có thể sinh ra dữ liệu trùng lặp hoặc chưa bao phủ hết các tổ hợp kiểm thử quan trọng.

Phương pháp GA giúp tối ưu tập dữ liệu bằng cách chọn lọc các test case có chất lượng tốt và tạo ra các test case mới thông qua lai ghép, đột biến. Kết quả sau GA thường có độ đa dạng tốt hơn và giảm bớt các dữ liệu trùng lặp.

Phương pháp kết hợp GA và HC cho kết quả tốt hơn vì tận dụng được ưu điểm của cả hai thuật toán. GA giúp tìm kiếm toàn cục và tạo ra tập test data đa dạng, còn HC giúp tinh chỉnh cục bộ các test case tốt nhất, đặc biệt là các giá trị tại biên hoặc gần biên. Từ kết quả so sánh, có thể nhận thấy hướng tiếp cận kết hợp LLM, GA và HC phù hợp với bài toán tự động sinh dữ liệu kiểm thử. Cách kết hợp này giúp giảm công sức tạo dữ liệu thủ công, tăng tính đa dạng của test data và hỗ trợ phát hiện lỗi hiệu quả hơn trong quá trình kiểm thử phần mềm.
...
### Biểu đồ trực quan hóa kết quả thực nghiệm

#### 1. Biểu đồ đường "So sánh tiến trình tối ưu" (Fitness Progress over generations)
Biểu đồ đường này thể hiện sự biến thiên của Điểm thích nghi tối ưu (Fitness Score) qua 30 thế hệ tiến hóa của 4 cấu hình chạy khác nhau:
- **Chỉ dùng LLM (LLM Only)**: Đường nằm ngang ở mức thấp (~0.22).
- **Chỉ dùng Leo đồi (HC Only)**: Tăng vọt trong 1-2 thế hệ đầu tiên rồi đi ngang ở mức trung bình (~0.51) do dễ bị kẹt ở tối ưu cục bộ.
- **Chỉ dùng Di truyền (GA Only)**: Tăng đều đặn theo dạng bậc thang qua các thế hệ (~0.78).
- **Tối ưu phối hợp (GA + HC - Memetic)**: Tăng cực nhanh ở các thế hệ đầu và tiệm cận sát mốc tuyệt đối (0.96 - 0.98).

```mermaid
xychart-beta
    title "So sánh tiến trình tối ưu qua 30 thế hệ (Fitness Progress)"
    x-axis ["Gen 0", "Gen 5", "Gen 10", "Gen 15", "Gen 20", "Gen 25", "Gen 30"]
    y-axis "Điểm Fitness" 0.0 --> 1.0
    line [0.22, 0.22, 0.22, 0.22, 0.22, 0.22, 0.22]
    line [0.22, 0.49, 0.51, 0.51, 0.51, 0.51, 0.51]
    line [0.22, 0.45, 0.58, 0.68, 0.74, 0.77, 0.78]
    line [0.22, 0.65, 0.81, 0.89, 0.94, 0.96, 0.98]
```

#### 2. Biểu đồ cột so sánh độ phủ biên và tính dư thừa

```mermaid
xychart-beta
    title "So sánh độ phủ biên (Boundary Coverage %) giữa các phương pháp"
    x-axis ["Đăng ký tài khoản", "Cổng thanh toán", "API Search"]
    y-axis "Độ phủ biên (%)" 0 --> 100
    bar [18.0, 12.5, 22.0]
    bar [72.0, 68.5, 75.0]
    bar [96.5, 98.2, 97.0]
```
...
```mermaid
xychart-beta
    title "So sánh tỷ lệ trùng lặp dữ liệu (%) giữa các phương pháp"
    x-axis ["Đăng ký tài khoản", "Cổng thanh toán", "API Search"]
    y-axis "Tỷ lệ trùng lặp (%)" 0 --> 50
    bar [42.0, 48.0, 35.5]
    bar [12.5, 15.0, 10.0]
    bar [2.2, 1.8, 2.5]
```
