...
const TECHNIQUES: { id: string; title: string; desc: string; recommended?: boolean }[] = [
  {
    id: 'ep',
    title: 'Phân vùng tương đương (EP)',
    desc: 'Nhóm dữ liệu đầu vào thành các phân vùng hành xử tương tự để giảm dư thừa.',
    recommended: true,
  },
  {
    id: 'bva',
    title: 'Phân tích giá trị biên (BVA)',
    desc: 'Kiểm thử các điểm biên của phân vùng — nơi lỗi dễ xuất hiện nhất.',
  },
  {
    id: 'random',
    title: 'Chọn ngẫu nhiên (Random)',
    desc: 'Sinh ngẫu nhiên các giá trị để kiểm tra độ tin cậy của hệ thống.',
  },
];
...
export const AnalysisDesign: React.FC = () => {
  ...
  const constraintCount = parsedSchema.filter(
    (f) =>
      f.minValue != null ||
      f.maxValue != null ||
      f.minLength != null ||
  ...
  const estimated = Math.min(
    60,
    (techniques.length || 1) * 12 + Math.max(0, parsedSchema.length - 3) * 4
  );
  ...
  const addField = () => {
    ...
    if (!name) return;
    if (parsedSchema.some((f) => f.name.toLowerCase() === name.toLowerCase())) {
      toast.warning('Tên trường đã tồn tại.');
      return;
    }
    setParsedSchema((prev) => [
      ...prev,
      { name, type: 'string', required: true, description: '' } as FieldConstraint,
    ]);
    setNewName('');
  };
  ...
  const handleGenerate = async () => {
    if (!hasSchema) {
      if (!rawText.trim()) {
        toast.warning('Chưa có đặc tả. Hãy nhập ở bước Đầu vào, hoặc bấm "Phân tích bằng AI".');
        return;
      }
      await handleParseSpec();
    }
    // Ánh xạ kỹ thuật đã chọn sang phương pháp sinh F0
    const map: Record<string, 'random' | 'bva' | 'ep' | 'decision'> = {
      ep: 'ep',
      bva: 'bva',
      decision: 'decision',
      state: 'random',
    };
    const methods = techniques.map((t) => map[t]).filter(Boolean) as (
      | 'random'
      | 'bva'
      | 'ep'
      | 'decision'
  ...
  return (
    <div className='fade-in-up'>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: 'minmax(0, 7fr) minmax(300px, 5fr)',
          gap: 20,
          alignItems: 'start',
        }}
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
          ...
          >
            ...
            <div style={{ padding: 18 }}>
              ...
              >
                Function:{' '}
                {schemaName ? schemaName.replace(/\s+/g, '_').slice(0, 28) : 'PENDING_PIPELINE'}
              </div>
              ...
              {isParsing && !hasSchema ? (
                ...
                >
                  <LoadingSpinner
                    size={70}
                    icon='sparkles'
                    outerColor='var(--brand-primary)'
                    innerColor='var(--color-teal)'
                  />
                  <div>
                    <h5
                      style={{
                    ...
                    >
                      Hệ thống đang xử lý ngôn ngữ tự nhiên để tự động trích xuất các quy tắc ràng
                      buộc logic & hạt giống dữ liệu.
                    </p>
                  </div>
                </div>
              ) : !hasSchema ? (
                <div
                  style={{ textAlign: 'center', padding: '28px 10px', color: 'var(--text-muted)' }}
                >
                  <Wand2 size={26} style={{ opacity: 0.5, marginBottom: 8 }} />
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
                    {rawText.trim()
                      ? 'Đặc tả đã sẵn sàng — phân tích để trích xuất ràng buộc.'
                      : 'Chưa có đặc tả. Hãy nhập ở bước Đầu vào.'}
                  </div>
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                    <button
                      className='btn btn-primary'
                      onClick={() => handleParseSpec(false)}
                      disabled={isParsing || !rawText.trim()}
                    >
                      {isParsing ? (
                        <RefreshCw size={15} className='tech-spinner' />
                      ) : (
                        <Sparkles size={15} />
                      )}
                    ...
                    {hasSchema && (
                      <button
                        ...
                        onClick={() => handleParseSpec(true)}
                        disabled={isParsing || !rawText.trim()}
                        title='Bỏ qua cache và phân tích mới hoàn toàn từ AI'
                      >
                        <RefreshCw size={14} />
                ...
                <>
                      ...
                      <div key={i} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        ...
                        >
                          ...
                          <select
                            value={f.type}
                            onChange={(e) =>
                              updateField(i, { type: e.target.value as FieldConstraint['type'] })
                            }
                            style={{
                          ...
                          <button
                            ...
                            style={{
                              ...
                              cursor: 'pointer',
                              color:
                                expandedRow === i ? 'var(--brand-primary)' : 'var(--text-muted)',
                              display: 'flex',
                              alignItems: 'center',
                              justifyContent: 'center',
                            }}
                          ...
                          >
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                >
                                  MẪU BIỂU THỨC CHÍNH QUY (REGULAR EXPRESSION)
                                </span>
                                <input
                                  type='text'
                                  value={f.regex ?? ''}
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                >
                                  DANH SÁCH GIÁ TRỊ CHO PHÉP (ENUM - PHÂN TÁCH BẰNG DẤU PHẨY)
                                </span>
                                <input
                                  type='text'
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                >
                                  CÁC GIÁ TRỊ MẪU HỢP LỆ (SUCCESS VALUES)
                                </span>
                                <input
                                  type='text'
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                >
                                  CÁC GIÁ TRỊ MẪU LỖI KÈM LÝ DO (FAILURE VALUES - JSON)
                                </span>
                                <input
                                  type='text'
                  ...
                  <div style={{ marginTop: 16 }}>
                    ...
                    >
                      1. Nếu dữ liệu hợp lệ &amp; thỏa ràng buộc →{' '}
                      <b style={{ color: 'var(--color-emerald)' }}>SUCCESS</b>
                      <br />
                      2. Nếu vi phạm biên/định dạng →{' '}
                      <b style={{ color: 'var(--color-rose)' }}>VALIDATION_ERROR</b>
                    </div>
                  </div>
                  ...
                  {(parsedBusinessRules.length > 0 || parsedConstraints.length > 0) && (
                    <div style={{ marginTop: 16 }}>
                      ...
                      >
                        {parsedBusinessRules.map((br, idx) => {
                          ...
                          return (
                            <div key={`br-${idx}`} style={{ marginBottom: 6 }}>
                              ...
                              {ruleDesc}
                              {errMsg && (
                                <span style={{ color: 'var(--color-rose)', marginLeft: 4 }}>
                                  (Lỗi: {errMsg})
                                </span>
                              )}
                            </div>
                        ...
                        {Array.from(
                          new Map(
                            parsedConstraints.map((c) => [
                              c.rule || c.description || JSON.stringify(c),
                              c,
                            ])
                          ).values()
                        ).map((c: any, idx) => {
                          ...
                          return (
                            <div key={`c-${idx}`} style={{ marginBottom: 6 }}>
                              <strong style={{ color: 'var(--color-violet)' }}>
                                Ràng buộc chéo ({label}):
                              </strong>{' '}
                              {ruleDesc}
                            </div>
                          );
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            <StatChip
              ...
              label='Inputs'
              value={`${parsedSchema.length} Params`}
            />
            <StatChip
              icon={<ListChecks size={16} />}
              label='Constraints'
              value={`${constraintCount} Nodes`}
            />
          ...
          >
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 11.5, color: 'var(--text-muted)', margin: '4px 0 0' }}>
                Chọn một hoặc nhiều kỹ thuật để sinh dữ liệu kiểm thử.
              </p>
            </div>
              ...
              {techniques.includes('bva') && (
                ...
                >
                  ...
                  >
                    ...
                    <option value={6}>
                      6 Biên (min-1, min, min+1, max-1, max, max+1) - Dò quét sâu
                    </option>
                  </select>
                </div>
              )}
            ...
            >
              ...
              >
                <span>Ước lượng số test case:</span>
                <b style={{ color: 'var(--brand-primary)' }}>~{estimated} Cases</b>
              </div>
              <button
                className='btn btn-primary'
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            ...
            <div>
              <h6 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>
                AI Assistant đang hoạt động
              </h6>
              <p style={{ fontSize: 12.5, color: 'var(--text-secondary)', margin: '2px 0 0' }}>
                Theo dõi các chỉnh sửa thủ công của bạn để tối ưu gợi ý về sau.
              </p>
            </div>
          </div>
          <button
            className='btn btn-secondary'
            style={{ fontSize: 12.5 }}
            onClick={() => setActiveScreen('evaluate')}
          >
            Xem Đánh Giá Dữ Liệu
          </button>
        </div>
      </div>
